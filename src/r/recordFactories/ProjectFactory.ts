...
import { FormResponse } from "../recordTypes/FormResponse";
...
import { QueryProperty } from "../recordTypes/Query";
...
export class ProjectFactory extends RecordFactory<RT.project> {
  ...
  getMetadata(): string[] {
    ...
    const metaArray = [
      this.get(FormResponse.description),
      this.get(FormResponse.project_start_description),
      this.get(FormResponse.project_end_description)
    ];
    ...
    for (const menuItem of this.getRecords(RT.menu)) {
      ...
      if (menuItemR.get(QueryProperty.menu_show) === true) {
        metaArray.push(menuItemR.get(QueryProperty.menu_display_name));
      }
    }
